...
- [x] Bilingual TR/EN Excel exports + Turkish-glyph PDF hakkediş certificate, all auth-guarded: submission ledger Excel with active filters and canonical record shape, three-sheet hakkediş Excel (Yeşil Defter / Fiyat İcmali / Hesap Özeti), worker + auditor performance summary Excel (OE excluded per D-110), and a hakkediş PDF rendered with embedded DejaVu Sans for full Turkish glyph coverage; all four route handlers explicitly call `auth()` first and return 401 JSON on no session — *Validated in Phase 11: Exports (EXP-01..EXP-04). 33/33 phase-level tests + 341/341 full suite green; TypeScript clean; 5/5 must-haves verified; 4 human visual gates approved live during Wave 6 UATs (11-HUMAN-UAT.md status: resolved). Distributed trigger surface (D-108) shipped via both the Exports hub page and the period-detail controls.*
- [x] Submission-driven hakkediş — each approved Telegram work-application submission contributes to the in-progress draft hakkediş period in real time (D-117 scoped recompute on `(project, boq_item, currency)` via shared `recomputeHakedisLine` helper); every hakkediş line-item quantity is traceable back to the source submission(s) via the new `hakedis_line_submissions(period_line_id, submission_id, qty_contributed)` join table with the corresponding `getLineSubmissions` Server Action; office sees updates within 30s via `LivePeriodPoller` (`router.refresh()` poll on draft-status pages only); v2.0 finalize/deduction/snapshot contract preserved unchanged — *Validated in Phase 12: Submission-Driven Hakkediş (SDH-01..SDH-03). Migration 0009 applied to dev + test DBs; 10/10 hakedis-live tests pass; 74/74 v2.0 regression suite stays green proving SDH-03 byte-identical contract; 4/4 must-haves verified; 3 manual UAT rows approved live. Pitfall 5 (no `logOfficeActivity` from bot path) honored.*
- [x] Dashboard-wide UX & brand pass — every existing surface (Phases 1–12: admin shell, overview, hakkediş, exports, analytics, people, records, settings, projects, auth, marketing root) re-skinned to the bayrak.ai brand language locked in 13-CONTEXT.md (D-121..D-128: slate + amber palette, Geist Sans + Geist Mono, wordmark logo with amber `.ai` suffix, `rounded-md` + compact + flat layout, Procore/Autodesk industry-utility density with amber as the deliberate departure from construction-software-blue); 7 shared brand component primitives in `src/components/brand/` (BrandButton, BrandCard, BrandHeading, BrandBadge, BrandEmpty, BrandLogo, BrandTable); brand reference checked in at repo-root `BRAND.md`; dynamic favicon + OG image + branded 404 + branded error boundary; nested i18n keys for `meta.not_found` + `meta.error` in TR + EN; no functional regression to v1/v2/v3.0 capabilities (358 vitest tests pass; Phase 11 PDF byte-identical; Phase 12 LivePeriodPoller null-on-disabled + sr-only span preserved with new visible `<BrandBadge variant="info">` sibling added per D-127 W2) — *Validated in Phase 13: UX & Brand Pass (BRAND-01..BRAND-03). 5 plans across 4 waves; key win is the Wave 1 token-cascade strategy: overriding shadcn semantic slots in `:root` propagates amber/slate to every Phase 1–12 surface without per-component edits. 4/4 must-haves verified; end-of-phase UAT covering 5 blocks (Wave 4 visual + TR↔EN + LivePeriodPoller dual-element + audit→hakkediş loop + suite+build) signed off 2026-05-29. Known follow-up: BoqTab/RouteTab/PeopleTab child components under `projects/[id]/page.tsx` retain pre-existing shadcn imports (out-of-scope per executor scope-boundary rule); resolvable via `/gsd:code-review 13 --fix`. v3.0 milestone complete.*
- [x] Office Engineer authenticates to the dashboard via email magic-link (Auth.js); worker and auditor Telegram User IDs are registered and assigned to projects in the dashboard — *Validated in Phase 1: Foundation (AUTH-01..AUTH-04, SETUP-01..SETUP-04, I18N-02).*
- [x] Office Engineer manages projects and Bill of Quantities (BOQ items) in a Next.js dashboard; can upload the pipeline route as a GeoJSON LineString and view remaining BOQ balance per line item — *Validated in Phase 1: Foundation (SETUP-01..SETUP-04).*
- [x] Worker submits a daily work log via a step-by-step conversational Telegram bot (photo → location share → quantity → notes → confirm), guided sequentially with reprompts on skipped/invalid steps; identified by Telegram User ID; selects active project from inline keyboard — *Validated in Phase 2: Worker Bot (LOG-01..LOG-10, I18N-01).*
- [x] Bot enforces input types: photo required (reject text), native Telegram location share required, numeric quantity required; on confirmation, submission persists with `status: pending_audit` and a webhook pings the assigned auditor — *Validated in Phase 2: Worker Bot (LOG-04, LOG-05, LOG-06, LOG-08).*
- [x] Auditor receives a Telegram summary (photo, location/map link, quantity, notes) with inline [✅ Approve] and [❌ Reject] buttons; on Approve the submission becomes `approved` and the BOQ line auto-decrements atomically; on Reject the auditor provides a reason, status becomes `rejected`, and the worker is notified; first-action-wins race safety for multiple auditors — *Validated in Phase 3: Audit Loop (AUDIT-01..AUDIT-06).*
- [x] Worker bot operates in Turkish; office dashboard is TR/EN switchable — *Validated in Phase 2: Worker Bot (I18N-01) and Phase 1: Foundation (I18N-02).*

### Active

- [ ] AI assist (vision/LLM) flags photo/location anomalies and auto-classifies submitted work to help the auditor decide — *Phase 16 scope (AI-01..AI-05); not yet built.*

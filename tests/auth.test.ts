 ...
 * Tests the exported `isAllowed(email): boolean` helper from src/lib/auth-allowlist.ts.
 * auth.ts re-exports this helper, so testing auth-allowlist is equivalent.
 */
...
describe('isAllowed — allowlist enforcement (AUTH-01 / D-11)', () => {
  ...
  it('returns false for a non-allowlisted email when list is set', async () => {
    ...
    const { isAllowed } = await import('../src/lib/auth-allowlist');
